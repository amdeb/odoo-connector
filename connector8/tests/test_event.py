...
class test_event(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.consumer1 = lambda model_name: None
        self.consumer2 = lambda model_name: None
        self.event = Event()
    ...
    def test_replacing_decorator(self):
        @self.event
        def consumer1(model_name):
            pass

        @self.event(replacing=consumer1)
        def consumer2(model_name):
            pass
    ...
    def test_fire(self):
        ...
        @self.event
        def set_message(model_name, recipient, message):
    ...
    def test_fire_several_consumers(self):
        ...
        @self.event
        def set_message(model_name, message):
            recipient.set_message(message)

        @self.event
        def set_message2(model_name, message):
            recipient2.set_message(message)

        # an event is fired on a model name
        self.event.fire('res.users', 'success')
        self.assertEquals(recipient.message, 'success')
    ...
    def test_consumer_uninstalled_module(self):
        ...
        # and we use odoo_module_name attribute of the consumer
        # thus set it as not existed
