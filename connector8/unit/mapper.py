...
class ImportMapChild(MapChild):
    ...
    def _child_mapper(self):
        return self.get_connector_unit_for_model(ImportMapper,
                                                 self.model._name)
...
class Mapper(ConnectorUnit):
    ...
    def _after_mapping(self, result):
        """ .. deprecated:: 2.1 """
        raise DeprecationWarning('Mapper._after_mapping() has been '
                                 'deprecated, use Mapper.finalize()')

    def convert_child(self, record, parent_values=None):
        """ .. deprecated:: 2.1 """
        raise DeprecationWarning('Mapper.convert_child() has been '
                                 'deprecated, use Mapper.map_record() then '
                                 'map_record.values() ')
